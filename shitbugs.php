...
if (!in_array($loguser['id'], [175, 1]) && $loguser['powerlevel'] < 1) {
    echo "
		$header<br>
		$tblstart
			<tr>$tccell1>&nbsp;
...
$expower = in_array($loguserid, [175, 1, 2100]);
...
while ($data = $sql->fetch($query)) {
    if (($lastip != $data['ip'] || $lastflag != $data['banflags']) && $lastflag != 0) {
        ++$rowcnt;
        echo str_replace('%%%COMBO%%%', $combocount > 1 ? " &times;$combocount" : '', $tempout);

        if (!($rowcnt % 50)) {
            echo $colheaders;
        } elseif ($lastip != $data['ip']) {
            echo "<tr>$tccellh colspan='$cells'><img src='images/_.gif' height=5 width=5></td></tr>";
        }
