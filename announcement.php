...
if (@mysql_num_rows(mysql_query("SELECT user FROM forummods WHERE forum=$f and user=$loguserid"))) {
    $ismod = 1;
}
...
if ($canpost) {
    ...
    if ($_GET['action'] == 'edit') {
        if (!$annc['headid']) {
            ...
            $head = mysql_result(mysql_query("SELECT text FROM postlayouts WHERE id=$annc[headid]"), 0, 0);
        }
        if (!$annc['signid']) {
            $sign = $annc['signtext'];
        } else {
            $sign = mysql_result(mysql_query("SELECT text FROM postlayouts WHERE id=$annc[signid]"), 0, 0);
        }
        sbr(1, $annc['text']);
        sbr(1, $head);
        sbr(1, $sign);
