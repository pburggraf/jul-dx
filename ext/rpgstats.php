...
if (!intval($u)) {
    exit('Missing ?u=<id> parameter');
}
$user = mysql_fetch_array(mysql_query("SELECT name,posts,regdate,users_rpg.* FROM users,users_rpg WHERE id='$u' AND uid=id")) or exit("User doesn't exist");
$p = $user[posts];
