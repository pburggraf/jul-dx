...
$specialscheme = '';
$smallbrowsers = ['Nintendo DS', 'Android', 'PSP', 'Windows CE', 'iPhone', 'Mobile'];
...
{
    return $posts == 0 ? 0 : sprintf('%01.3f', 172800 * @(@pow(@($days / $posts), 0.5) / $posts));
}
...
function ctime()
{
    return time() + CTIME_ADJUSTMENT;
}
function cmicrotime()
{
    return microtime(true) + CTIME_ADJUSTMENT;
}
...
{
    switch ($t) {
        case 0: $src = htmlspecialchars($src);
            break;
        case 1: $src = urlencode($src);
            break;
...
function sbr($t, &$src)
{
    global $br;
    switch ($t) {
        case 0: $src = str_replace($br, '<br>', $src);
            break;
        case 1: $src = str_replace('<br>', $br, $src);
            break;
    }
}
...
function errorpage($text, $redir = '', $redirurl = '')
{
    ...
    echo "{$header}<br>{$tblstart}{$tccell1}>{$text}";

    if ($redir) {
        echo '<br>'.redirect($redirurl, $redir, 0);
    }
...
{
    return "<marquee scrollamount='". mt_rand(1, 50) ."' scrolldelay='". mt_rand(1, 50) ."' direction='". pick_any(['left', 'right']) ."'>$str</marquee>";
}
