...
function threadpost($post, $bg, $pthread = '')
{
    ...
    if ($post['location']) {
        $set['location'] = 'From: '. htmlspecialchars($post['location']);
    }
    if ($post['pronouns']) {
        $set['pronouns'] = 'Pronouns: '. htmlspecialchars($post['pronouns']);
    }
    ...
    if ($post['picture'] || ($post['moodid'] && $post['moodurl'])) {
        ...
        if (($post['moodid'] ?? false) && ($post['moodurl'] ?? false)) {
            $set['userpic'] = "<img class='avatar' src=\"". str_replace(['$', '>', '"'], [$post['moodid'], '%3E', '%22'], $post['moodurl']) .'">';
            $set['picture'] = str_replace(['$', '>', '"'], [$post['moodid'], '%3E', '%22'], $post['moodurl']);
        }
    }
...
function syndrome($num, $double = false, $bar = true)
{
    ...
    if ($num >= 800) {
        $syn = "62C7F2$a 'Xkeeper Syndrome' +++++!!"; /*	$last= 600; $next= 200;		}
