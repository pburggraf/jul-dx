...
echo "
	...
	";
if ($isadmin) {
    echo '<br>Admin cruft: <a href=online.php'. ($sort ? '?sort=1&' : '?') ."time=$time>Sort by ".($sort == 'IP' ? 'date' : 'IP') .'</a>';
}
...
echo "<br>
	...
	";
if ($isadmin) {
    echo "$tccellh width=120>IP address</td>";
}
echo "$tccellh width=60> Posts</tr>";
