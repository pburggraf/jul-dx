...
if (!$log) {
    ...
    switch ($action) {
	  ...
	  ";
            $atrlist = '';
            for ($i = 0; $i < 9; ++$i) {
                $atrlist .= "$tccellh width=50>$stat[$i]</td>";
            }
	  ...
	  ";
            while ($item = mysql_fetch_array($items)) {
                ...
                if ($item[id] == $eqitem[id]) {
                    $color = ' class=equal';
                } elseif ($item[coins] > $GP || $item[gcoins] > $user['gcoins']) {
                ...
                for ($i = 0; $i < 9; ++$i) {
                    ...
                    if (!$color && substr($item[stype], $i, 1) == substr($eqitem[stype], $i, 1)) {
                        if ($itst > $eqst) {
                            $st = "<font class=higher>$st</font>";
                        }
                        if ($itst == $eqst) {
                            $st = "<font class=equal>$st</font>";
                        }
                        if ($itst < $eqst) {
                            $st = "<font class=lower>$st</font>";
                        }
                    }
                    $atrlist .= "
		  $tccell1>$st</td>";
        ...
        case 'buy':
            ...
            if ($item && $item[coins] <= $GP && $item['gcoins'] <= $user['gcoins']) {
                ...
                echo 'Debug output: Cost: '. $item['coins'] ." - Current item's sell value: ". ($pitem['coins'] * 0.6) .' - Amount to subtract: '. $whatever .'. /debug';
