...
if ($_POST['edit'] || $_POST['edit2']) {
    ...
    if (!isset($_GET['delete']) || $id < 0) {
        exit('No forum selected to delete.');
    }
    if (!isset($_POST['mergeid']) || $mergeid < 0) {
        exit('No forum selected to merge to.');
    }

    $counts = $sql->fetchq("SELECT `numthreads`, `numposts` FROM `forums` WHERE `id`='$id'");
    $sql->query("UPDATE `threads` SET `forum`='$mergeid' WHERE `forum`='$id'") or exit(mysql_error());
    $sql->query("UPDATE `announcements` SET `forum`='$mergeid' WHERE `forum`='$id'") or exit(mysql_error());
    $sql->query("DELETE FROM `forummods` WHERE `forum`='$id'") or exit(mysql_error());
    $sql->query("DELETE FROM `forums` WHERE `id`='$id'") or exit(mysql_error());
	...
	WHERE `id`='$mergeid'") or exit(mysql_error());
...
admincheck();
echo adminlinkbar('admin-editforums.php');
...
echo '<center><b>Preview forums with powerlevel:</b> '.previewbox()."</center>\n";
echo "$tblstart$forumlist$tblend$footer";
