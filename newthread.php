...
if (!$_POST['action'] or $_POST['paction']) {
    ...
    if ($log and $forums[$id]['minpowerthread'] > $power) {
        echo "$tccell1>Sorry, but you are not allowed to post";
        if ($banned) {
            echo ', because you are banned from this board.<br>'.redirect("forum.php?id=$id", 'return to the forum', 0);
        } else {
            echo ' in this restricted forum.<br>'.redirect('index.php', 'return to the board', 0);
        }
    } else {
        if ($log) {
...
if ($_POST['action'] == 'postthread' and !$_POST['paction']) {
    ...
    if ($userid != -1 && $subject && $message && $forumexists && $authorized && $limithit) {
        ...
        if ($submit) {
            ...
            if ($pid) {
                mysql_query("INSERT INTO `posts_text` (`pid`, `text`, `tagval`, `options`) VALUES ('$pid', '$msg', '$tagval', '$options')");
            }
