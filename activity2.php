...
$user['regdate'] = $sql->resultq('SELECT MIN(`regdate`) FROM users WHERE regdate > 0') or exit;
...
while ($u = $sql->fetch($userq)) {
    $users[$u['id']] = ['name' => $u['name'], 'color' => imagecolorallocate($img, rand(100, 255), rand(100, 255), rand(100, 255))];
}
