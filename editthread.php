...
if (substr($_GET['action'], 0, 1) == 'q') {
    switch ($_GET['action']) {
        case 'qstick':   $update = 'sticky=1';
            break;
        case 'qunstick': $update = 'sticky=0';
            break;
        case 'qclose':   $update = 'closed=1';
            break;
...
elseif ($_GET['action'] == 'trashthread') {
    ...
    echo "$header<br><FORM ACTION=editthread.php NAME=REPLIER METHOD=POST>$tblstart
