...
echo "$header<br>";
admincheck();
echo adminlinkbar('ipsearch.php');
echo "$tblstart";
...
if ($ip) {
    $ip = str_replace('*', '%', $ip);
    switch ($su) {
        case 'n': $usort = 'ORDER BY name';
            break;
        case 'p': $usort = 'ORDER BY posts DESC';
            break;
        case 'r': $usort = 'ORDER BY regdate';
            break;
        case 's': $usort = 'ORDER BY lastposttime';
            break;
        case 'a': $usort = 'ORDER BY lastactivity';
            break;
        case 'i': $usort = 'ORDER BY lastip';
            break;
    }
    switch ($sp) {
        case 'u': $psort = 'ORDER BY name';
            break;
        case 'd': $psort = 'ORDER BY date';
            break;
        case 'i': $psort = 'ORDER BY ip';
            break;
    }
    switch ($sm) {
        case 'n': $msort = 'ORDER BY name1';
            break;
        case 'd': $msort = 'ORDER BY date';
            break;
        case 'i': $msort = 'ORDER BY ip';
            break;
    }
    if ($d === 'y') {
    ...
    if ($post = $sql->fetch($users)) {
        echo "<tr>$tccell2 colspan=7>Too many results!";
    }
    ...
    if ($post = $sql->fetch($posts)) {
        echo "<tr>$tccell2 colspan=5>Too many results!";
    }
    ...
    if ($pmsg = $sql->fetch($pmsgs)) {
        echo "<tr>$tccell2 colspan=6>Too many results!";
    }
}
echo $tblend.$footer;
