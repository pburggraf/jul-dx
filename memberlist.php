...
if (!in_array($sort, ['name', 'reg', 'exp', 'age', 'posts', 'act'])) {
    $sort = 'posts';
}
...
if ($sort == 'name') {
    $users1 = $sql->query("$query$where ORDER BY name", MYSQL_ASSOC);
}
if ($sort == 'reg') {
    $users1 = $sql->query("$query$where ORDER BY regdate DESC", MYSQL_ASSOC);
}
if ($sort == 'act') {
    $users1 = $sql->query("$query$where ORDER BY lastactivity DESC", MYSQL_ASSOC);
}
if ($sort == 'exp') {
    $users1 = $sql->query("$query$where", MYSQL_ASSOC);
}
if ($sort == 'age') {
    $users1 = $sql->query("$query$where AND birthday ORDER BY birthday", MYSQL_ASSOC);
}
if ($sort == 'posts') {
    $users1 = $sql->query("$query$where ORDER BY posts DESC", MYSQL_ASSOC);
}
...
if (!$rpg) {
    ...
    echo "$tccellh width=35>Level</td>";
    echo "$tccellh width=90>Class</td>";
    for ($i = 0; $i < 9; ++$i) {
        echo "$tccellh width=65>".$stat[$i].'</td>';
    }
    echo "$tccellh width=80><img src=images/coin.gif></td>";
    echo "$tccellh width=60><img src=images/coin2.gif></td>";
    echo '</tr>';
}
...
for ($u = 0; $u < $ppp; ++$u) {
    ...
    if (!$rpg) {
        ...
        for ($k = 0; $k < 9; ++$k) {
            $ulist .= "$tccell1s>".$stats[$stat[$k]].'</td>';
        }
...
echo "$ulist$tblend$pagelinks$footer";
