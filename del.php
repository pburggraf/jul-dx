...
if (!in_array($_SERVER['REMOTE_ADDR'], $allowedusers)) {
    exit('Nein.');
}
...
if ($_POST['deluser'] and $isadmin) { // ($loguserid==1 or $loguserid==2)){
    foreach ($_POST['deluser'] as $id => $junk) {
        ...
        while ($user = mysql_fetch_array($user2)) {
            ...
            while ($up = mysql_fetch_array($ups)) {
                mysql_query("UPDATE posts_text SET signtext=CONCAT_WS('','$line',signtext) WHERE pid=$up[id]") or print mysql_error();
            }
