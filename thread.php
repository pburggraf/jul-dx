...
if ($id) {
    do {
        ...
        if ($tnext) {
            $tlinks[] = "<a href='?id={$tnext}' style='white-space:nowrap;'>Next newer thread</a>";
        }
        ...
        if ($tprev) {
            $tlinks[] = "<a href='?id={$tprev}' style='white-space:nowrap;'>Next older thread</a>";
        }
...
// temporary
if ($windowtitle) {
    $windowtitle = "$boardname -- $windowtitle";
}
require_once 'lib/layout.php';
...
if ($thread_error) {
    switch ($thread_error) {
        case E_BADPOSTS: $errortext = 'This thread does not exist, but posts exist that are associated with this invalid thread ID.';
            break;
        case E_BADFORUM: $errortext = 'This thread has an invalid forum ID; it is located in a forum that does not exist.';
            break;
    }
...
switch ($loguser['viewsig']) {
    case 1:  $sfields = ',headtext,signtext';
        break;
    case 2:  $sfields = ',u.postheader headtext,u.signature signtext';
        break;
    default: $sfields = '';
        break;
}
...
if ($forumid) {
    ...
    if (!$thread['closed']) {
        $threadforumlinks .= " - <a href=newreply.php?id=$id>$newreplypic</a>";
    } else {
        $threadforumlinks .= " - $closedpic";
    }
}
