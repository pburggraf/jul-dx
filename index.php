...
if (false && $x_hacks['smallbrowse'] == 1) {
    ...
    $onlineusers = '';
    if ($onlineusersa) {
        $onlineusers = ': '. implode(', ', $onlineusersa);
    }
    ...
    if ($posts['d'] > $misc['maxpostsday']) {
        $sql->query("UPDATE misc SET maxpostsday=$posts[d],maxpostsdaydate=".ctime());
    }
    ...
    if ($log) {
        ...
        if ($totalpms) {
            ...
            $namelink = getuserlink($pmsg, ['id' => 'uid']);
    ...
    if ($log) {
        ...
        foreach ($forums as $forum) {
            $qadd[] = "(lastpostdate > '". ($postread[$forum['id']] ?? 0) ."' AND forum = '{$forum['id']}')\r\n";
        }
    ...
    foreach ($categories as $category) {
        ...
        foreach ($forums as $forumplace => $forum) {
            ...
            $namelink = getuserlink($forum, ['id' => 'uid']);
            if ($forum['numposts']) {
