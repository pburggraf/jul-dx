...
if (mysql_num_rows($threads) <= 0) {
    ...
    for ($i = 1; $thread = @$sql->fetch($threads, MYSQL_ASSOC); ++$i) {
        ...
        if (trim($thread['title']) == '') {
            ...
            $thread['title'] = str_replace(['<', '>'], ['&lt;', '&gt;'], trim($thread['title']));
        }

        $threadtitle = "<a href='thread.php?id=$thread[id]'>$thread[title]</a>";
        $belowtitle = []; // An extra line below the title in certain circumstances
        ...
        $userlink1 = getuserlink($thread,
            ['sex' => 'sex1', 'powerlevel' => 'pwr1', 'id' => 'user',       'aka' => 'aka1', 'name' => 'name1', 'birthday' => 'bd1']);
        $userlink2 = getuserlink($thread,
            ['sex' => 'sex2', 'powerlevel' => 'pwr2', 'id' => 'lastposter', 'aka' => 'aka2', 'name' => 'name2', 'birthday' => 'bd2']);
